...
class Player(AdvancedHitbox): # p
	...
	def handle_keys(self, keys_down: list[bool], hb_mouse: Hitbox, delta: float, walls: list[Surface], teleporters: list[Teleporter]) -> None:
		...
		if not self.can_fly:
			...
			if keys_down[K_SPACE] and self.is_grounded and not self.jumped_while_sliding:
				self.vec_move.y -= 500
				self.is_grounded = False
				self.space_was_down = False
				if self.is_sliding:
					self.jumped_while_sliding = True
			elif keys_down[K_SPACE] and not self.is_grounded and self.can_double_jump and self.space_was_down and not self.jumped_while_sliding:
		...
		for wall in walls:
			if p_temp.check_collisions(wall):
				...
				if self.vec_move.y > 0:
					...
					self.vec_move.x = self.vec_move.x + (self.vec_move.x * wall.friction) * 60 * delta
					if abs(self.vec_move.x) < .08:
						self.set_is_sliding(False, walls, delta, keys_down)
						self.vec_move.x = 0
						self.jumped_while_sliding = False
...
class Button(Hitbox):
	def __init__(self, pt: Vector, w: float, h: float, text: str, has_border: bool, location: str, font: pygame.font, color: str = "#ff0000"):
		super().__init__(pt, w, h, color)
		self.text = text
